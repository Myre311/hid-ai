...
export function scoreComputerVision(rawAnswers, { sessionId } = {}) {
  ...
  // Mélange précision/rappel (65/35) au lieu d'une division brute par le
  // nombre total de vérités : un débutant qui annote correctement les
  // objets principaux n'est plus massacré pour avoir manqué la longue
  // traîne de petites instances. La précision (qualité des box dessinées)
  // pèse plus que le rappel (exhaustivité), tout en gardant une incitation
  // à annoter davantage. Courbe douce IoU/0.7 conservée.
  let totalIou = 0;
  let imageCount = 0;
  CV_GROUND_TRUTH.forEach((img) => {
    ...
    // Rappel : crédit par vérité (meilleure box utilisateur), / nb vérités
    let recallCredit = 0;
    truth.forEach((tBox) => {
      const bestIou = user.reduce(
        (max, uBox) => Math.max(max, iouBox(tBox, uBox)),
        0
      );
      recallCredit += Math.min(1, bestIou / 0.7);
    });
    const recall = recallCredit / truth.length;

    // Précision : crédit moyen des box réellement dessinées vs vérité la
    // plus proche (récompense la justesse, pas l'endurance).
    let precision = 0;
    if (user.length > 0) {
      let precSum = 0;
      user.forEach((uBox) => {
        const bestIou = truth.reduce(
          (max, tBox) => Math.max(max, iouBox(tBox, uBox)),
          0
        );
        precSum += Math.min(1, bestIou / 0.7);
      });
      precision = precSum / user.length;
    }

    totalIou += 0.65 * precision + 0.35 * recall;
  });
  ...
  // Object tracking (35 points) — 50 frames disponibles, mais on note la
  // COMPÉTENCE montrée, pas l'ENDURANCE : le score plein est atteint dès
  // ~15 frames correctement suivies (objectId correct + IoU >= 0.4).
  // Suivre une personne sur 50 frames à la main dans un test de 1re étape
  // était irréaliste et universellement abandonné.
  const TRACKING_TARGET_FRAMES = 15;
  let goodFrames = 0;
  ...
  const trackingPts =
    35 * Math.min(1, goodFrames / TRACKING_TARGET_FRAMES);
