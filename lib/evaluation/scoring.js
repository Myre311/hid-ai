...
import { CV_GROUND_TRUTH, CV_POLYGON_TARGET } from "./data/cv-ground-truth";
...
export function scoreComputerVision(rawAnswers) {
  ...
  const iouPts = iouRatio * 65;

  // Attributs (20 points) — ratio de boxes avec l'attribut attendu
  let attrCorrect = 0;
  let attrTotal = 0;
  CV_GROUND_TRUTH.forEach((img) => {
    const user = userBoxes[img.id] || [];
  ...
  const total = iouPts + attrPts + polyPts;
  return {
    score: clampScore(total),
    precision_rate: Number((total / 100).toFixed(4)),
    cases_processed: imageCount + (polygon.length > 0 ? 1 : 0),
  };
}
