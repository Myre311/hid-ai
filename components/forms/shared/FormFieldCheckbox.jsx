"use client";

import { Check } from "lucide-react";
import { cn } from "@/lib/utils/cn";

/**
 * Checkbox group multi-sélection.
 * value = array of selected values.
 * onChange(newArray)
 *
 * options = [{ value, label, description?, icon? }]
 */
export function FormFieldCheckbox({
  label,
  name,
  value = [],
  onChange,
  options,
  error,
  required,
  hint,
  variant = "list", // "list" | "cards"
  cols = 2,
}) {
  const toggle = (v) => {
    if (value.includes(v)) onChange(value.filter((x) => x !== v));
    else onChange([...value, v]);
  };

  return (
    <fieldset className="flex flex-col gap-2">
      {label && (
        <legend className="text-sm text-foreground/70 mb-1">
          {label}
          {required && <span className="text-red-400 ml-1">*</span>}
        </legend>
      )}
      {hint && <p className="text-xs text-foreground/40">{hint}</p>}

      {variant === "cards" ? (
        <div
          className={cn(
            "grid gap-3 mt-1",
            cols === 2 && "md:grid-cols-2",
            cols === 3 && "md:grid-cols-3"
          )}
        >
          {options.map((o) => {
            const Icon = o.icon;
            const checked = value.includes(o.value);
            return (
              <label
                key={o.value}
                className={cn(
                  "flex items-start gap-3 p-4 rounded-lg cursor-pointer transition-all border",
                  checked
                    ? "border-accent bg-accent/5"
                    : "border-white/10 bg-[#1A1A1A] hover:border-white/30"
                )}
              >
                <input
                  type="checkbox"
                  name={name}
                  value={o.value}
                  checked={checked}
                  onChange={() => toggle(o.value)}
                  className="sr-only"
                />
                <span
                  className={cn(
                    "h-5 w-5 rounded border-2 flex-shrink-0 mt-0.5 flex items-center justify-center",
                    checked
                      ? "border-accent bg-accent"
                      : "border-white/30"
                  )}
                >
                  {checked && <Check className="h-3 w-3 text-background" strokeWidth={3} />}
                </span>
                {Icon && (
                  <Icon
                    className={cn(
                      "h-5 w-5 flex-shrink-0 mt-0.5",
                      checked ? "text-accent" : "text-foreground/60"
                    )}
                    aria-hidden="true"
                  />
                )}
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span className="text-sm font-medium text-foreground">
                    {o.label}
                  </span>
                  {o.description && (
                    <span className="text-xs text-foreground/55">
                      {o.description}
                    </span>
                  )}
                </div>
              </label>
            );
          })}
        </div>
      ) : (
        <div className="flex flex-col gap-2 mt-1">
          {options.map((o) => {
            const checked = value.includes(o.value);
            return (
              <label
                key={o.value}
                className={cn(
                  "flex items-center gap-3 p-3 rounded-md cursor-pointer transition-colors border",
                  checked
                    ? "border-accent/60 bg-accent/5"
                    : "border-white/10 bg-[#1A1A1A] hover:border-white/25"
                )}
              >
                <input
                  type="checkbox"
                  name={name}
                  value={o.value}
                  checked={checked}
                  onChange={() => toggle(o.value)}
                  className="sr-only"
                />
                <span
                  className={cn(
                    "h-4 w-4 rounded border-2 flex-shrink-0 flex items-center justify-center",
                    checked ? "border-accent bg-accent" : "border-white/30"
                  )}
                >
                  {checked && (
                    <Check className="h-3 w-3 text-background" strokeWidth={3} />
                  )}
                </span>
                <span className="text-sm text-foreground">{o.label}</span>
              </label>
            );
          })}
        </div>
      )}

      {error && <span className="text-xs text-red-400">{error}</span>}
    </fieldset>
  );
}

/**
 * Checkbox simple inline (1 seule, pour un consent / une condition).
 */
export function FormCheckboxSingle({
  label,
  checked,
  onChange,
  required,
  error,
  hint,
}) {
  return (
    <div className="flex flex-col gap-1">
      <label className="flex items-start gap-3 cursor-pointer">
        <input
          type="checkbox"
          checked={!!checked}
          onChange={(e) => onChange(e.target.checked)}
          className="sr-only"
        />
        <span
          className={cn(
            "h-4 w-4 rounded border-2 flex-shrink-0 mt-0.5 flex items-center justify-center",
            checked ? "border-accent bg-accent" : "border-white/30"
          )}
        >
          {checked && <Check className="h-3 w-3 text-background" strokeWidth={3} />}
        </span>
        <span className="text-sm text-foreground/85 leading-relaxed">
          {label}
          {required && <span className="text-red-400 ml-1">*</span>}
        </span>
      </label>
      {hint && <p className="text-xs text-foreground/40 ml-7">{hint}</p>}
      {error && <p className="text-xs text-red-400 ml-7">{error}</p>}
    </div>
  );
}
